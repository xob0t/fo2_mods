...
- Split orientation is selected from the in-game split party setup. The choice applies without restarting and persists in `fo2_splitscreen_layout.lua`; missing or invalid state defaults to `Vertical`, while native hook failure safely forces `Horizontal`.
...
The script/native bridge uses two private numeric events rather than overloading a game-flow or player field. A scoped constructor hook at `0x0048D2F5` supplies process-lifetime descriptors only for IDs `0x7F020001` and `0x7F020002`; all stock IDs use the original registry lookup. A second hook at `0x0048D4C0` recognizes those two fully built events, changes the in-memory orientation, and swallows them before the stock queue. Every other event replays the original comparison and queue call. The selected value is atomically stored as the fixed Lua chunk `fo2_splitscreen_layout.lua` (`return 0` or `return 1`); the party script reads it with `loadfile`/`pcall` immediately after each request so a native fallback is reflected in the menu. A runtime change is committed in memory only after persistence succeeds; a write failure keeps the previous orientation. A missing or invalid file defaults to vertical. Delete this generated state file to reset the selector to its default; it is not a packaged asset or user-facing INI option.

The mount, one feature-scoped startup-slot hook, the ready-owner gate, four mode-scoped routing hooks, the post-processing dispatch hook, and the two orientation-event bridge hooks form one signature-checked ten-site transaction. The implementation validates the pinned executable layout, PE timestamp, all ten complete instruction sequences, the private event IDs, and both support files before its first write. It installs the mount last, verifies every branch, and restores every earlier site if installation fails. With `SplitscreenPostProcessingFix=0`, the post-processing dispatch replays the exact single stock call.
