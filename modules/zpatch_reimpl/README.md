...
The in-game `SPLIT LAYOUT` selector controls a separate seven-site, signature-checked native transaction. That transaction is installed for both orientations so switching applies immediately: it wraps the stock viewport builder call at `0x0045CF1B`, then rewrites the two records as left/right halves only when `Vertical` and `GM_SPLITSCREEN` are both active. Player 1 receives `floor(width/2)` columns and player 2 receives the remainder, so odd display widths lose no pixels. A render latch is re-derived from the exact live viewport records after every builder call, so later projection and HUD passes remain correct after the setup-mode field changes. The stock count-two aspect sites use `0.5` for left/right, widescreen extents consume the authoritative viewport aspect at projection `+0x118`, the singleton race minimap is centered on the full 640-unit HUD canvas, and player 2's POSITION background/title/value use their normal top anchor instead of the horizontal split bottom override. Horizontal, single-player, and derby behavior remains stock. If any complete signature fails, the orientation transaction rolls back and the selector remains safely horizontal.
