...
The in-game `SPLIT LAYOUT` selector controls a separate four-site, signature-checked native transaction. That transaction is installed for both orientations so switching applies immediately: it wraps the stock viewport builder call at `0x0045CF1B`, then rewrites the two records as left/right halves only when `Vertical` and `GM_SPLITSCREEN` are both active. Player 1 receives `floor(width/2)` columns and player 2 receives the remainder, so odd display widths lose no pixels. A render latch records that the validated vertical rewrite actually ran; this remains authoritative for later projection and HUD passes even after the setup-mode field changes. The two independent stock count-two aspect doublers at `0x004C9E27` and `0x004CBD5D` use that latch: horizontal keeps the stock `2.0` scale, while vertical uses `0.5`. Widescreen projection extents use the native effective viewport aspect already stored at projection offset `+0x118`, avoiding a second split correction. The fourth hook centers the one shared race minimap on the 640-unit full-screen HUD canvas in vertical mode while leaving its Y position and all horizontal, single-player, and derby behavior stock. If any complete signature fails, the orientation transaction rolls back and the selector remains safely horizontal.
