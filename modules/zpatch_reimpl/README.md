...
The in-game `SPLIT LAYOUT` selector controls a separate seven-site, signature-checked native transaction. That transaction is installed for both orientations so switching applies immediately: it wraps the stock viewport builder call at `0x0045CF1B`, then rewrites the two records as left/right halves only when `Vertical` and `GM_SPLITSCREEN` are both active. Player 1 receives `floor(width/2)` columns and player 2 receives the remainder, so odd display widths lose no pixels. Visual state is derived from the authoritative live viewport rectangles after every builder call and again in HUD helpers, so a menu selection cannot leave projection or HUD behavior on a stale lifecycle latch. The stock count-two aspect sites use `0.5` for left/right, widescreen extents consume the authoritative viewport aspect at projection `+0x118`, and the singleton race minimap is centered on the full 640-unit HUD canvas. In vertical mode the two translucent POSITION background bars are suppressed and both players' POSITION title/value use the normal top anchor; horizontal, single-player, and derby behavior remains stock. If any complete signature fails, the orientation transaction rolls back and the selector remains safely horizontal.
