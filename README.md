...
- `SplitscreenFix` is an opt-in two- or three-player prototype. The party roster includes indexed input-device selectors, stores one distinct zero-based keyboard/pad index per player, uses the stock keyboard-plus-two-pad topology for 3P, and keeps its four routing hooks on stock paths outside `GM_SPLITSCREEN`. Three players use the top-left, top-right, and bottom-left quadrants of a fixed 2x2 grid.
